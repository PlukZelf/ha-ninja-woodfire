...
async def dump_services(address: str) -> list[ServiceDump]:
...
async def listen(address: str, characteristics: list[str], timeout: float) -> None:
    async with BleakClient(address) as client:
        active: list[str] = []
...
async def main() -> None:
    ...
    services = await dump_services(args.address)
    if args.json:
        print_json(services)
    else:
        print_text(services)
